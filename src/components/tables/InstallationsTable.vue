...
<v-card>
    <v-toolbar card color="white">
        <v-text-field flat solo prepend-icon="search" :placeholder="this.$i18n.t('dataTables.installationsTable.search')" v-model="searchInstallations" hide-details class="hidden-sm-and-down"></v-text-field>
        <v-btn icon>
    ...
    <v-card-text class="pa-0">
        <v-data-table :headers="headers" :search="searchInstallations" :items="installationsItems" :rows-per-page-items="[10,25,50,{text:'All','value':-1}]" :pagination.sync="pagination" class="elevation-1" item-key="id">
            <template slot="items" slot-scope="props">
                <tr @click.stop="navigateTo('/installation/' + props.item.id)" :style="{ cursor: 'pointer'}">
