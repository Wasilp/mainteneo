...
<v-card>
    ...
    <v-card-text class="pa-0">
        <v-data-table :headers="headers" :search="search" :items="tanksItems" :rows-per-page-items="[10,25,50,{text:'All','value':-1}]" :pagination.sync="pagination" class="elevation-1" item-key="id" v-model="selected" select-all>
            <template slot="items" slot-scope="props">
                <tr @click.stop="navigateTo('/tank/' + props.item.id)" :style="{ cursor: 'pointer'}">
                    ...
                    <td class>
                        ...
                        <span v-else-if="props.item.currentType==2">{{ $t('dropdownMenus.tankType.transfer') }}</span>
                        <span v-else-if="props.item.currentType==3">{{ $t('dropdownMenus.tankType.recovery') }}</span>
                    </td>
                    <td class>{{ props.item.capacity }} kg</td>
                    <td class>{{ props.item.quantity }} kg</td>
                    <td class style="padding:10px">
                        <v-progress-circular :size="76" :width="10" :rotate="-90" :value="props.item.quantity/props.item.capacity*100" color="accent">{{ Math.round((props.item.quantity / props.item.capacity) *100) }} %</v-progress-circular>
                    </td>
