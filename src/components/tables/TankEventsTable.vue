  ...
  <v-card>
    <v-toolbar card color="white">
      <v-text-field
        ...
        solo
        prepend-icon="search"
        :placeholder="this.$i18n.t('dataTables.tanksEventTable.search')"
        v-model="search"
        hide-details
      ...
      >
        <template slot="items" slot-scope="props">
          ...
          <td class>
            ...
            <span
              v-else-if="props.item.actionType == 3"
            >{{ $t('dropdownMenus.tankEventType.release') }}</span>
            <span
              v-else-if="props.item.actionType == 4"
            >{{ $t('dropdownMenus.tankEventType.releaseOnSite') }}</span>
            <span
              v-else-if="props.item.actionType == 5"
            >{{ $t('dropdownMenus.tankEventType.toSupplier') }}</span>
            <span
              v-else-if="props.item.actionType == 6"
            >{{ $t('dropdownMenus.tankEventType.toUnstock') }}</span>
          </td>
          ...
          <td class nowrap>
            <div v-if="props.item.actionType == 2">
              <div v-if="props.item.eventQuantity < 0" key="quantityPositive">
                ...
                {{ Math.abs(props.item.eventQuantity) }} kg {{ $t('dataTables.tanksEventTable.headers.injected') }}
              </div>
              <div v-else key="quantityNegative">
                <v-icon color="#D32F2F" small>arrow_downward</v-icon>
                {{ props.item.eventQuantity }} kg {{ $t('dataTables.tanksEventTable.headers.recovered') }}
              </div>
            </div>
          </td>
          <td class>
